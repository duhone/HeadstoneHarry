...
SpriteInternal::SpriteInternal(bool _singleSetMode, int _z) : m_singleSetMode(_singleSetMode), m_lighting(false), m_visible(true),
	m_textureId(NOT_ASSIGNED),m_oldTextureId(NOT_ASSIGNED), z_position(_z), m_zAngle(0) ,m_uOffset(0), m_vOffset(0)
{
	assert(_z >= 0 && _z <= 1000);
	pause = false;
...
void SpriteInternal::Render(vector<CR::Graphics::Vertex> &_vertices,std::vector<GLushort> &_indices)
{
	...
	topLeft.X(topLeft.X()+x_position-160);
	topLeft.Y(topLeft.Y()+240-y_position);
	topRight.X(topRight.X()+x_position-160);
	topRight.Y(topRight.Y()+240-y_position);
	bottomLeft.X(bottomLeft.X()+x_position-160);
	bottomLeft.Y(bottomLeft.Y()+240-y_position);
	bottomRight.X(bottomRight.X()+x_position-160);
	bottomRight.Y(bottomRight.Y()+240-y_position);
