...
Sweeper::Sweeper(int _z) : m_z(_z), m_texture(NULL), m_imageNumber(NOT_ASSIGNED), m_sweep(1.0f),m_transitionWidth(0)
{	
	m_indices[0] = 0;
	m_indices[1] = 1;
	m_indices[2] = 2;
...
void Sweeper::RenderActual()
{
	...
	int xstart = m_position.X()-160-(m_texture->width>>1);
	int ystart = 240-m_position.Y()-(m_texture->height>>1);
