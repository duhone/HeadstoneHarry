...
string SaveInfo::GetStringForKey(NSString *key) const
{
	NSString *tmp = [saveData objectForKey:key];
	string str = [tmp UTF8String];
	//[tmp release];
	return str;
}

void SaveInfo::SetStringForKey(string _value, NSString *key)
{
	NSString *str = [[NSString alloc] initWithCString:_value.c_str()];
	[saveData setValue:str forKey:key];
	[str release];
}
