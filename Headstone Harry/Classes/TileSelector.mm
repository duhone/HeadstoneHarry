...
TileSelector::TileSelector(MatchThreePuzzle *parentPuzzle)
{
	m_parentPuzzle = parentPuzzle;
	m_visible = true;
	
	m_selectorSprite = graphics_engine->CreateSprite1(true,800);
	m_selectorSprite->SetImage(CR::AssetList::Selector_2); // TODO: Change back to selector_1
...
void TileSelector::SetPosition(CR::Math::PointI position)
{
	...
	if (position.X() > (MatchThreePuzzle::x_tiles - 1) || position.X() < 0 ||
		position.Y() > (MatchThreePuzzle::y_tiles - 1) || position.Y() < 0)
		return;
