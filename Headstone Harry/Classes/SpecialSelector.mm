...
SpecialSelector::SpecialSelector(MatchThreePuzzle *parentPuzzle)
{
...
void SpecialSelector::SetTilePosition(int x, int y)
{
	...
	if (x > (MatchThreePuzzle::x_tiles - 1) || x < 0 ||
		y > (MatchThreePuzzle::y_tiles - 1) || y < 0)
		return;
	
	SetPosition(x * PuzzleTile::tile_width + PuzzleTile::tile_width/2 + (x * m_parentPuzzle->Spacing()) + m_parentPuzzle->Offset_X(),
				 y * PuzzleTile::tile_height + PuzzleTile::tile_height/2 + (y * m_parentPuzzle->Spacing()) + m_parentPuzzle->Offset_Y());
}
